...
*/
        ...
        case WM_LBUTTONUP: {
            ...
            if (bUrl) {
                LPSTR tmpStr;
                HRESULT hresult = ::TranslateURL(Url, TRANSLATEURL_FL_GUESS_PROTOCOL | TRANSLATEURL_FL_GUESS_PROTOCOL,
                                                 &tmpStr);
                if (hresult == S_OK)
...
//		dwRes = Configure( (HWND) lParam1, (LPDRVCONFIGINFO) lParam2 );
        ...
        default:
            ...
            my_debug.OutPut(DEBUG_LEVEL_MSG,
                            "ACM::DriverProc unknown message (0x%08X), lParam1 = 0x%08X, lParam2 = 0x%08X", msg,
                            lParam1, lParam2);
    ...
    switch (a_Query & ACM_FORMATDETAILSF_QUERYMASK) {
        ...
        case ACM_FORMATDETAILSF_INDEX :
            my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE, "enter ACM_FORMATDETAILSF_INDEX for index 0x%04X:%03d",
                            a_FormatDetails->dwFormatTag, a_FormatDetails->dwFormatIndex);
            if (a_FormatDetails->dwFormatTag == PERSONAL_FORMAT) {
                if (a_FormatDetails->dwFormatIndex < GetNumberEncodingFormats()) {
                    ...
                    my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE, "format in  : channels %d, sample rate %d",
                                    WaveExt->nChannels, WaveExt->nSamplesPerSec);
                    GetMP3FormatForIndex(a_FormatDetails->dwFormatIndex, *WaveExt, a_FormatDetails->szFormat);
                    my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE, "format out : channels %d, sample rate %d",
                                    WaveExt->nChannels, WaveExt->nSamplesPerSec);
                ...
                my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE, "Unknown a_FormatDetails->dwFormatTag = 0x%08X",
                                a_FormatDetails->dwFormatTag);
            }
        ...
        case ACM_FORMATDETAILSF_FORMAT :
            ...
            my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE,
                            "enter ACM_FORMATDETAILSF_FORMAT : 0x%04X:%03d, format in : channels %d, sample rate %d",
                            a_FormatDetails->dwFormatTag, a_FormatDetails->dwFormatIndex, WaveExt->nChannels,
                            WaveExt->nSamplesPerSec);
    ...
    if (a_FormatTagDetails->cbStruct >= sizeof(*a_FormatTagDetails)) {
        ...
        switch (a_Query & ACM_FORMATTAGDETAILSF_QUERYMASK) {
            ...
            case ACM_FORMATTAGDETAILSF_INDEX:
                ...
                my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE, "get ACM_FORMATTAGDETAILSF_INDEX for index %03d",
                                a_FormatTagDetails->dwFormatTagIndex);
                ...
                if (a_FormatTagDetails->dwFormatTagIndex < FORMAT_TAG_MAX_NB) {
                    ...
                    my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE, "ACM_FORMATTAGDETAILSF_INDEX for unsupported index %03d",
                                    a_FormatTagDetails->dwFormatTagIndex);
                    Result = ACMERR_NOTPOSSIBLE;
                }
                break;
            ...
            case ACM_FORMATTAGDETAILSF_FORMATTAG:
                ...
                my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE,
                                "get ACM_FORMATTAGDETAILSF_FORMATTAG for index 0x%02X, cStandardFormats = %d",
                                a_FormatTagDetails->dwFormatTagIndex, a_FormatTagDetails->cStandardFormats);
                break;
        ...
        my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE, "OnFormatTagDetails %d possibilities for format 0x%08X",
                        a_FormatTagDetails->cStandardFormats, the_format);
    } else {
...
inline DWORD ACM::OnFormatSuggest(LPACMDRVFORMATSUGGEST a_FormatSuggest) {
    ...
    my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE,
                    "Suggest for source format = 0x%04X, channels = %d, Samples/s = %d, AvgB/s = %d, BlockAlign = %d, b/sample = %d",
    ...
    my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE,
                    "Suggested destination format = 0x%04X, channels = %d, Samples/s = %d, AvgB/s = %d, BlockAlign = %d, b/sample = %d",
    ...
    switch (a_FormatSuggest->pwfxSrc->wFormatTag) {
        case WAVE_FORMAT_PCM:
            ...
            a_FormatSuggest->pwfxDst->nBlockAlign =
                    a_FormatSuggest->pwfxDst->nChannels * a_FormatSuggest->pwfxDst->wBitsPerSample / 8;
        ...
        case PERSONAL_FORMAT:
            ...
            a_FormatSuggest->pwfxDst->nAvgBytesPerSec =
                    a_FormatSuggest->pwfxDst->nSamplesPerSec * a_FormatSuggest->pwfxDst->nChannels *
                    a_FormatSuggest->pwfxDst->wBitsPerSample / 8;
...
inline DWORD ACM::OnStreamOpen(LPACMDRVSTREAMINSTANCE a_StreamInstance) {
    ...
    switch (a_StreamInstance->pwfxSrc->wFormatTag) {
        case WAVE_FORMAT_PCM:
            my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE,
                            "Open stream for PCM source (%05d samples %d channels %d bits/sample)",
                            a_StreamInstance->pwfxSrc->nSamplesPerSec, a_StreamInstance->pwfxSrc->nChannels,
                            a_StreamInstance->pwfxSrc->wBitsPerSample);
            if (a_StreamInstance->pwfxDst->wFormatTag == PERSONAL_FORMAT) {
                ...
                my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE,
                                "Open stream for PERSONAL output (%05d samples %d channels %d bits/sample %d kbps)",
                                a_StreamInstance->pwfxDst->nSamplesPerSec, a_StreamInstance->pwfxDst->nChannels,
                                a_StreamInstance->pwfxDst->wBitsPerSample,
                                8 * a_StreamInstance->pwfxDst->nAvgBytesPerSec);
        ...
        case PERSONAL_FORMAT:
            my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE,
                            "Open stream for PERSONAL source (%05d samples %d channels %d bits/sample %d kbps)",
                            a_StreamInstance->pwfxSrc->nSamplesPerSec, a_StreamInstance->pwfxSrc->nChannels,
                            a_StreamInstance->pwfxSrc->wBitsPerSample, 8 * a_StreamInstance->pwfxSrc->nAvgBytesPerSec);
...
inline DWORD ACM::OnStreamSize(LPACMDRVSTREAMINSTANCE a_StreamInstance, LPACMDRVSTREAMSIZE the_StreamSize) {
    ...
    switch (ACM_STREAMSIZEF_QUERYMASK & the_StreamSize->fdwSize) {
        case ACM_STREAMSIZEF_DESTINATION:
            my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE, "Get source buffer size for destination size = %d",
                            the_StreamSize->cbDstLength);
            break;
        case ACM_STREAMSIZEF_SOURCE:
            my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE, "Get destination buffer size for source size = %d",
                            the_StreamSize->cbSrcLength);
...
inline DWORD ACM::OnStreamPrepareHeader(LPACMDRVSTREAMINSTANCE a_StreamInstance, LPACMSTREAMHEADER a_StreamHeader) {
    ...
    my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE, "  prepare : Src : %d (0x%08X) / %d - Dst : %d (0x%08X) / %d",
                    a_StreamHeader->cbSrcLength, a_StreamHeader->pbSrc, a_StreamHeader->cbSrcLengthUsed,
                    a_StreamHeader->cbDstLength, a_StreamHeader->pbDst, a_StreamHeader->cbDstLengthUsed
    );
...
inline DWORD ACM::OnStreamUnPrepareHeader(LPACMDRVSTREAMINSTANCE a_StreamInstance, LPACMSTREAMHEADER a_StreamHeader) {
    ...
    my_debug.OutPut(DEBUG_LEVEL_FUNC_CODE, "unprepare : Src : %d / %d - Dst : %d / %d", a_StreamHeader->cbSrcLength,
                    a_StreamHeader->cbSrcLengthUsed, a_StreamHeader->cbDstLength, a_StreamHeader->cbDstLengthUsed
    );
    if (WAVE_FORMAT_PCM == a_StreamInstance->pwfxSrc->wFormatTag &&
        PERSONAL_FORMAT == a_StreamInstance->pwfxDst->wFormatTag) {
        ACMStream *the_stream = (ACMStream *) a_StreamInstance->dwInstance;
...
void ACM::GetMP3FormatForIndex(const DWORD the_Index, WAVEFORMATEX &the_Format,
    ...
    if (the_Index < bitrate_table.size()) {
        ...
        if (bitrate_table[the_Index].mode == vbr_abr)
            wsprintfA(temp, "%d Hz, %d kbps ABR, %s", the_Format.nSamplesPerSec, the_Format.nAvgBytesPerSec * 8 / 1000,
                      (the_Format.nChannels == 1) ? "Mono" : "Stereo");
        else
            wsprintfA(temp, "%d Hz, %d kbps CBR, %s", the_Format.nSamplesPerSec, the_Format.nAvgBytesPerSec * 8 / 1000,
                      (the_Format.nChannels == 1) ? "Mono" : "Stereo");
...
void ACM::GetPCMFormatForIndex(const DWORD the_Index, WAVEFORMATEX &the_Format,
                               unsigned short the_String[ACMFORMATDETAILS_FORMAT_CHARS]) const {
    the_Format.nChannels = SIZE_CHANNEL_MODE - int(the_Index % SIZE_CHANNEL_MODE);
    the_Format.wBitsPerSample = 16;
    ...
    for (channel = 0; channel < SIZE_CHANNEL_MODE; channel++) {
        ...
        for (freq = 0; freq < SIZE_FREQ_MPEG1; freq++) {
            for (bitrate = 0; bitrate < SIZE_BITRATE_MPEG1; bitrate++) {
                ...
                if (!my_EncodingProperties.GetSmartOutputMode() ||
                    IsSmartOutput(mpeg1_freq[freq], mpeg1_bitrate[bitrate], channel + 1)) {
        ...
        for (freq = 0; freq < SIZE_FREQ_MPEG2; freq++) {
            for (bitrate = 0; bitrate < SIZE_BITRATE_MPEG2; bitrate++) {
                if (!my_EncodingProperties.GetSmartOutputMode() ||
                    IsSmartOutput(mpeg2_freq[freq], mpeg2_bitrate[bitrate], channel + 1)) {
                    bitrate_item bitrate_table_tmp;
        ...
        for (channel = 0; channel < SIZE_CHANNEL_MODE; channel++) {
            ...
            for (freq = 0; freq < SIZE_FREQ_MPEG1; freq++) {
                for (bitrate = my_EncodingProperties.GetAbrBitrateMax();
                    ...
                    if (bitrate >= mpeg1_bitrate[SIZE_BITRATE_MPEG1 - 1] &&
                        (!my_EncodingProperties.GetSmartOutputMode() ||
                         IsSmartOutput(mpeg1_freq[freq], bitrate, channel + 1))) {
                        bitrate_item bitrate_table_tmp;
            ...
            for (freq = 0; freq < SIZE_FREQ_MPEG2; freq++) {
                for (bitrate = my_EncodingProperties.GetAbrBitrateMax();
                     bitrate >= my_EncodingProperties.GetAbrBitrateMin();
                     bitrate -= my_EncodingProperties.GetAbrBitrateStep()) {
                    if (bitrate >= mpeg2_bitrate[SIZE_BITRATE_MPEG2 - 1] &&
                        (!my_EncodingProperties.GetSmartOutputMode() ||
                         IsSmartOutput(mpeg2_freq[freq], bitrate, channel + 1))) {
                        bitrate_item bitrate_table_tmp;
